...
#fhatlist=unlist(Ytrue)
#future ones 
#fhatlist=c(fhatlist,unlist(Ytrue))
#indicator=c(rep('0.25',50),rep('0.50',50),rep('0.75',50),
#rep('1',50),rep('2',50))

#ggplot()+geom_boxplot(aes(as.factor(indicator),fhatlist),col='purple')+xlab(expression(delta))+ylab(expression(hat(f)))




#MAKE BIGGER BOXPLOT 
#load('visualrho02.Rdata','visualrho05p100.Rdata',
#     'visualrho07p100.Rdata','visualrho09p100.Rdata')


#rhos=c(rho02,rho05,rho07,rho09)
#indicator=rep(c(rep('0.25',50),rep('0.50',50),rep('0.75',50),
#            rep('1',50),rep('2',50)),4)
#color=c(rep("0.2",250),rep("0.5",250),rep('0.7',250),rep('0.9',250))

#data=data.frame(rho=rhos,indicator=indicator, ρ =color)

#data_long = melt(data, id.vars=c("rho","indicator",'ρ'))

#ggplot(data_long) +  
#  geom_boxplot( aes(x = indicator, y = rhos,color = ρ))+ theme(legend.key.size = unit(2, 'cm'))+
#  xlab(expression(delta)) +ylab(expression(hat(f)))
  
  # Draw ggplot2 boxplot without colors
#  geom_boxplot( aes(x = indicator, y = rhos))
